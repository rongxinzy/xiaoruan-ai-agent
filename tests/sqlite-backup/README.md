...
2. 确认目标数据库路径。默认桌面版用户数据路径通常是：  
   ...
   ```bash
   ~/Library/Application\ Support/XiaoruanAgent/zhiyuan.sqlite
   ```
...
npm run test:sqlite-backup:seed -- \
  --db '/Users/jj.deng/Library/Application Support/XiaoruanAgent/zhiyuan.sqlite' \
  --sessions 10 \
...
```bash
ls -lh ~/Library/Application\ Support/XiaoruanAgent/backups/sqlite/snapshots/
```
...
```bash
printf 'not-a-sqlite-db' > ~/Library/Application\ Support/XiaoruanAgent/zhiyuan.sqlite
```
