#!/usr/bin/env node
/**
 * Expert Register for 晓软Agent.
 *
 * Validates an expert package and registers its agents into the 晓软Agent
 ...
 *
 * If --db-path is omitted, the script looks for the default 晓软Agent SQLite
...
function getDefaultDbPath() {
  ...
  let base;
  if (platform === 'win32') {
    // Electron's app.getPath('userData') resolves to the 晓软Agent directory on Windows.
  ...
  return path.join(base, 'XiaoruanAgent', DEFAULT_DB_FILENAME);
}
...
function loadBetterSqlite3() {
  try {
    // When running inside the 晓软Agent source tree, prefer project's better-sqlite3.
    return require('better-sqlite3');
  } catch (e) {
    throw new Error(
      'better-sqlite3 is not available. Make sure to run this script from the 晓软Agent project directory.',
    );
  }
}
