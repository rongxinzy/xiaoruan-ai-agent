...
# 安装器的 "Stopping running 晓软AI智能体 processes" 阶段对齐。
#
# 匹配范围(按可执行路径,避免误杀 dpkg 自身——其命令行含 .deb 文件名):
#   - 主进程与全部子进程(renderer/GPU/node sidecar/engram):可执行路径均
#     位于 deb 安装目录 /opt/晓软AI智能体/ 下
#   - 用户同时运行 AppImage 时,其挂载路径 /tmp/.mount_晓软AI智能体* 也匹配
#
# 注意:preinst 以 root 运行,不能依赖 $HOME;因此不清 SingletonLock 文件
# (Electron 启动时会检测残留 socket 并自动清理),只杀进程。

# Current packages install /opt/晓软AI智能体 and mount AppImages below
# /tmp/.mount_晓软AI智能体*. Keep the legacy XiaoruanAgent paths for shutdown during an
# upgrade. Both expressions match the executable/mount path, never the .deb
# filename passed to dpkg.
DEB_INSTALL_PATTERN='/opt/(晓软AI智能体|XiaoruanAgent)(/|$)'
APPIMAGE_MOUNT_PATTERN='\.mount_(晓软AI智能体|XiaoruanAgent)'
