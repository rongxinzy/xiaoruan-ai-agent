...
test('does not misclassify unrelated failures', () => {
  const cases: unknown[] = [
    ...
    new ApiError('tool_choice requires an object with a function name', 400),
    new ApiError('This model does not support tools', 401),
    new ApiError('This model does not support tools', 500),
  ];
  for (const error of cases) {
