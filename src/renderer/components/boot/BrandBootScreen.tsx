...
// 2026/09/17 lixiang  恢复晓软 logo 粒子汇聚启动屏（替代静态 BrandBootScreen）
...
const loadSampledLogo = (src: string): Promise<SampledLogo> =>
  new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      ...
      const h = Math.max(1, Math.round(image.naturalHeight * (ANALYSIS_WIDTH_PX / image.naturalWidth)));
...
export function BrandBootScreen({ exiting, onExitComplete }: BrandBootScreenProps) {
  ...
  return (
    <div
      ...
      aria-live="polite"
      className={cn(
        'relative min-h-0 flex-1 overflow-hidden bg-background transition-opacity duration-200',
