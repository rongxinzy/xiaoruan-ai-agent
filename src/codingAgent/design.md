...
本文档定义晓软智能体“编程”模式的产品边界、底层架构、Agent 发现与接入、多 Agent 协作、工作区安全、数据模型、UI 草图、实施阶段和验收标准。

后续实现应以本文档为基线。若实现过程中需要改变本文档中的协议边界、任务模型、写入隔离或交互语义，应先更新设计并重新评审，不应直接在代码中形成隐式新方案。

## 2. 一句话定义

“编程”是晓软智能体内置的统一编程工作台：用户即使没有安装任何外部编程 Agent，也能使用内置的晓软智能体编程 Agent 完成任务；安装了 Claude Code、Codex、OpenCode 等 Agent 后，晓软智能体通过 ACP 发现、连接并组织它们协作，但不接管或注入这些 Agent 的模型与账号。
...
- 内置晓软智能体编程 Agent 永远是第一方、一等能力。
- 外部 Agent 是可选增强项，不是使用编程模式的前置条件。
- 外部 Agent 通过 ACP 接入。
- 晓软智能体不向外部 Agent 注入自己的 API Key、Provider URL、模型名称、系统提示词或内置运行时配置。
- 每个外部 Agent 保留自己的账号、模型、配置、工具和记忆。
- 晓软智能体只使用 Agent 通过 ACP 明确声明的能力。

### 3.3 多 Agent 边界

- ACP 解决的是 Client 与 Agent 之间的连接，不直接定义 Agent 与 Agent 的协作协议。
- 多 Agent 协作由晓软智能体的 Coding Room Orchestrator 组织。
...
- UI 显示“晓软智能体编程 Agent”，不显示内部运行时名称。
...
ACP 是外部 Agent 的稳定集成边界，不必强制成为晓软智能体内部所有调用的唯一协议。
...
如果未来需要让第三方编辑器连接晓软智能体编程 Agent，可以另行提供 ACP Server Adapter，但这不属于第一阶段。
...
- 晓软智能体不能硬编码某个外部 Agent 一定支持某项能力。

`available_commands_update` 按 ACP 语义是某个 Session 的完整替换快照，不是增量事件。晓软智能体按
...
斜杠命令是外部 Agent 暴露其 MCP、Skill 和自身操作入口的标准 UI 通道。晓软智能体不解析或复制外部
Agent 的私有 Skill/MCP 配置，也不把晓软智能体的模型、Skill 或 MCP 注入外部 Agent。比如 Codex 或
...
Codex 与 Claude Code 作为首批一等外部 Agent，采用“用户安装 Agent、晓软智能体内置桥接”的交付方式：
...
晓软智能体编程 Agent 是默认 Agent，而不是外部 Agent 不可用时才出现的降级入口。
...
- 用户已配置可用模型时，可以直接开始任务。
- 如果晓软智能体本身尚未配置任何可用模型，菜单仍显示内置 Agent，但状态为“需要配置模型”，并引导用户完成晓软智能体自身配置。
- 外部 Agent 的认证状态不得影响内置 Agent。

### 7.3 配置边界

- 内置 Agent 使用晓软智能体现有模型、权限、Skills、MCP 和本地推理配置。
- 这些配置不复制给外部 Agent。
...
- 晓软智能体按需启动外部 Agent 或 Adapter 子进程。
...
一个 Agent Profile 的连接可以承载多个 ACP Session。每个 Session 必须有晓软智能体本地 ID 与远端 opaque session ID 的映射。
...
- 晓软智能体不使用自己的模型在幕后替用户决定哪个外部 Agent 应该做什么；协作编排按确定性规则或用户选择执行。
...
晓软智能体只在真正完成某个 Client Capability 时才向 Agent 声明支持，不能为了通过握手虚假声明文件系统、终端、认证或 Elicitation 能力。
...
```text
┌──────────────────────┬────────────────────────────────────┬───────────────────┐
│ 工作 / 对话          │ 晓软智能体 · Codex                │ Changes / Files   │
│ 新建任务             ├────────────────────────────────────┤ / Terminal        │
│ 本地推理             │                                    │                   │
...
Workspace 创建/编辑 Dialog 包含显示名称、默认 Agent 和一个或多个 SourceFolder。默认 Agent 是新建 Session 草稿的首选项，不会改变已有 Session 的绑定。添加文件夹只挂载现有目录；移除 Workspace 只删除晓软智能体中的编程记录，永远不删除磁盘文件。
...
- 不出现阻塞式安装引导。
- 首次进入时可以直接用晓软智能体编程 Agent 创建 Session。
...
- 用户不安装任何外部 Agent，也能用晓软智能体编程 Agent 完成编程任务。
- 能正确区分并展示外部 Agent 的发现、连接、认证和兼容状态。
- 切换 Agent 不丢失会话。
- 没有晓软智能体模型配置被注入外部 Agent。
...
- 把晓软智能体模型注入 Claude Code、Codex 或 OpenCode。
...
> 内置晓软智能体编程 Agent 保证零外部安装可用；ACP 负责连接用户已有的外部 Agent；Zed 式 Agent Picker 负责统一切入；Coding Room 负责会话、任务、权限、工作区和多 Agent 协作。
