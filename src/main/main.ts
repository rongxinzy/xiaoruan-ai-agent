...
/**
 * Linux only: 检测单实例锁被哪个"晓软智能体"实例持有。
 *
 ...
 *  - deb:/opt/晓软智能体 下的进程无 APPIMAGE,exe 路径匹配即视为同族;
 *    无法区分版本,统一按"旧实例"提示确认
 */
async function findStaleLinuxInstances(): Promise<{
  ...
  const familyPattern = /晓软智能体|XiaoruanAgent/i;
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (appImage) {
        ...
        // 非 AppImage:匹配 deb 安装路径 /opt/晓软智能体
        const exe = fs.readlinkSync(`/proc/${pid}/exe`);
        if (!/\/opt\/晓软智能体/.test(exe)) continue;
        pids.push(pid);
      }
    } catch {
      // 权限不足或进程已退出,跳过
  ...
  // 从 AppImage 文件名提取旧版本号,如 晓软智能体-1.0.0.AppImage → 1.0.0
  const versionMatch = oldAppImages[0]?.match(/-(\d+\.\d+\.\d+)\.AppImage/i);
  const oldLabel = versionMatch ? `晓软智能体 ${versionMatch[1]}` : '旧版本的晓软智能体';
    ...
    if (stale) {
      ...
      try {
        ...
        const { response } = await dialog.showMessageBox({
          ...
          detail:
            `当前启动的是晓软智能体 ${currentVersion}。启动新版本需要先关闭旧版本,` +
