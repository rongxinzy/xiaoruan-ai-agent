...
class StoredOAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    return { client_name: 'Xiaoruan Government Office Agent', redirect_uris: [this.redirectUri] };
  }
    ...
    try {
      ...
      const code = await new Promise<string>((resolve, reject) => {
        ...
        callbackServer.once('request', (request, response) => {
          ...
          response.end('<html><body><p>You can return to Xiaoruan Government Office Agent.</p></body></html>');
