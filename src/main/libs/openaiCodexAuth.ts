...
function renderCallbackHtml(success: boolean, message: string): string {
  ...
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>XiaoruanAgent · ChatGPT Login</title>
<style>
...
export function startOpenAICodexLogin(): Promise<CodexOAuthTokens> {
  ...
  return new Promise<CodexOAuthTokens>((resolve, reject) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      res.end(renderCallbackHtml(true, 'You can now close this tab and return to XiaoruanAgent.'));
