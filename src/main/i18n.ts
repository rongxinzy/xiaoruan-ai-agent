 ...
 *   setLanguage('en');
 *   const label = t('trayShowWindow'); // "Open 晓软智能体"
...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    localInferenceImportRuntimeDialogMessage:
      '请选择本地推理 backend 主包压缩包（zip 或 tar.gz）；如果已经解压，请进入目录后选择其中任意文件。晓软智能体 会校验平台和 backend 类型后导入。',
  ...
  en: {
    ...
    localInferenceImportRuntimeDialogMessage:
      'Select a local inference backend archive (zip or tar.gz). If it is already extracted, open that directory and choose any file inside it. 晓软Agent will validate the platform and backend type before importing it.',
    coworkErrorUnknown:
